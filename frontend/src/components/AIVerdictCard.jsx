function AIVerdictCard({ verdict }) {
  ...
  return (
    <div className={`bg-dark-card border ${colors.border} rounded-xl p-6`}>
      <div className="flex items-start justify-between mb-4">
        <div>
          ...
          <p className="text-muted text-sm mt-1">Powered by Claude AI</p>
        </div>
        <div className="flex items-center gap-2">
          <span className={`text-sm font-medium px-3 py-1 rounded-full ${colors.bg} ${colors.text}`}>
            {verdict.verdict}
          </span>
          <span className="text-xs text-muted bg-dark-border/50 px-2 py-1 rounded-full">
            {verdict.confidence} confidence
          </span>
        </div>
      </div>
      <p className="text-muted text-sm leading-relaxed">{verdict.explanation}</p>
    </div>
  )
}
