  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link
          to="/"
          className="text-sm bg-accent hover:bg-red-700 text-white font-semibold px-4 py-2 rounded-lg transition-colors"
        >
          New Analysis
        </Link>
      </div>
      ...
      {items.length === 0 ? (
        <div className="bg-dark-card border border-dark-border rounded-xl p-12 text-center">
          ...
          <p className="text-muted">No emails analyzed yet.</p>
        ...
        <div className="space-y-2">
          {items.map((item) => (
            <Link
              key={item.id}
              to={`/results/${item.id}`}
              className="block bg-dark-card border border-dark-border rounded-xl p-4 hover:bg-white/[0.02] transition-colors"
            >
              <div className="flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    <span className="text-muted text-xs truncate">{item.subject || 'No subject'}</span>
                    <span className="text-muted text-xs">&mdash;</span>
                    <span className="text-muted text-xs truncate">{item.sender}</span>
                  </div>
                </div>
